<script>
	...
	let inProgressCardsLocalStorage = JSON.parse(localStorage.getItem('inProgressCardsLocalStorage'))
	let doneCardsLocalStorage = JSON.parse(localStorage.getItem('doneCardsLocalStorage'))
	...
	function handleEventAddCard(event) {
		...
		if (data.listName == 'Tasks') {
			...
			inProgressCards = [...taskCards, {todo: data.todo}]
			localStorage.setItem('inProgressCards', JSON.stringify(inProgressCards))
		} else {
			doneCards = [...taskCards, {todo: data.todo}]
